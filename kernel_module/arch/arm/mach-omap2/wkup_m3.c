...
int wkup_m3_ping_delay(int iteration)
{
	...
	val_buf = 0xA55A0000 | iteration;

        if (!wkup_m3->mbox) {
                pr_err("PM: No IPC channel to communicate with wkup_m3!\n");
                return -EIO;
        }
        writel((u32)val_buf, wkup_m3->ipc + AM33XX_CONTROL_IPC_MSG_REG7);
         ...
         */
        ret = omap_mbox_msg_send(wkup_m3->mbox, 0xABCDABCD);
	while(readl(wkup_m3->ipc + AM33XX_CONTROL_IPC_MSG_REG7)){
	}
	time_end = ktime_get();
	diff = ktime_to_us(ktime_sub(time_end, time_start));
	return (int)diff;
}
EXPORT_SYMBOL_GPL(wkup_m3_ping_delay);
